...
grangerTlasso <-
  ...
  )
  {
    ...
    if (is.null(newEst))
    {
        newEst <- grangerLasso(X, d = d, 
                               group = group, 
                               typeIerr = typeIerr,
                               weights = weights)$estMat
    }
    X = X[ , ,(tp - d) : tp]
    for (i in 1:(d + 1))
    {
    ...
    while ( (diff > tol) && (iter <= maxIter))
    {
      ...
      while (jj <= d & CONTINUE)
      {
        idx = d + 1 - jj
        XX <- Matrix(X[ , ,idx])
        theta <- array2mat(newEst[ , , -idx])
        for (i in 1:p)
        {
          YY[,i] <- Y[,i] -
            array2mat(X[ , , - idx]) %*% theta[i,]
        }

        ## calculate truncating factor
        if (jj > 1)
        {
          previousNNZ <- sum(abs(newEst[,,(idx + 1)]) > thresh )/(p^2)
        ...
        if (CONTINUE)
        {
          if (!useAlasso)
          {
            ...
            newEst[,,idx] <- as.matrix(tempp$AA)
          }else{
            W <- (abs(weights[,,idx]) + eps*Onep)^(-Alasso.power)
            if (sum(W < 1) > 0)
            {
              W[(W < 1)] <- 1
            }
