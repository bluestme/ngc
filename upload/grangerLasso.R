...
grangerLasso <-
  function(
    ...
    ##scale the X matrix
    X = array(unlist(lapply(1:tp, function(x) scale( X[,,x] )*sqrt(n/(n-1)))), dim = dim(X))
    ...
    if (!useAlasso)
    {
      temp = pldag.set(XX, YY, group = group, sigLevel=typeIerr, wantScale=TRUE, lambda = lambda)
    }
    else
    {
      ...
      temp = pldag.set(XX, YY, group = group, sigLevel = typeIerr,
                      useWghts = TRUE, wghts = W, wantScale = TRUE, lambda = lambda)
    }
